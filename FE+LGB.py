...
def train_test(num_rows=None):
    ...
    #  sign out outliers
    train_df['outliers'] = 0
    train_df.loc[train_df['target'] < -20 , 'outliers'] = 1
    train_df.loc[train_df['target'] > 20, 'outliers'] = 1

    #选定target值小于-30的ID标记其为outlier，找到后面如何处理outlier的，首先要思考如何找出outlier
    ...
    for f in ['feature_1', 'feature_2', 'feature_3']:
        order_label = df.groupby([f])['outliers'].mean()  #按照f分组并计算outlier的均值。
...
def historical_transactions(num_rows=None):
    ...
    aggs['category_1'] = ['mean','var']
    aggs['category_2'] = ['mean','var']
    aggs['category_3'] = ['mean','var']
...
def kfold_lightgbm(train_df, test_df, num_folds, stratified=False, debug=False):
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(train_df[feats], train_df['outliers'])):
        ...
        oof_preds[valid_idx] = reg.predict(valid_x, num_iteration=reg.best_iteration)  #这个是对于valid集的输出，valid选用的是删除outlier的输出。
        ...
        a.append(rmse(valid_y, oof_preds[valid_idx]))
